  ...
  return function (state, segmento) {
    if ((0, _segmentoCategoria2.default)(segmento) === _categorias2.default.ESPECIALIDADE) {
      return _ramda2.default.over(_ramda2.default.lensProp('especialidade'), function (state) {
        return desenvolvimentoEspecialidade(state, marcacoesSegmento[segmento]);
      }, state);
    }
...
function desenvolvimentoEspecialidade(state, marcacoes) {
  return marcacoes.reduce(function (acc, _ref) {
    ...
    var qtdAtualRamo = acc[segmento] || 0;
    var qtdEspecialidadeAtual = (acc[id] || 0) + (type === MARKED ? 1 : -1);
    var ganhoRamo = calcularGanhoRamo(qtdEspecialidadeAtual, total, type);

    return _ramda2.default.merge(acc, (_R$merge2 = {}, _defineProperty(_R$merge2, id, qtdEspecialidadeAtual), _defineProperty(_R$merge2, segmento, qtdAtualRamo + ganhoRamo), _R$merge2));
  }, state);
}
